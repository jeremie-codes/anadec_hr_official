...
@section('title', 'Agents Malades - ANADEC RH')
@section('page-title', 'Agents Malades')
@section('page-description', 'Liste des agents en congé maladie')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border">
        <div class="flex items-center">
            <div class="w-12 h-12 bg-yellow-100 rounded-full flex items-center justify-center mr-4">
                <i class="bx bx-first-aid text-yellow-600 text-2xl"></i>
            </div>
            <div>
                <h3 class="text-2xl font-bold text-gray-900">{{ $agents->total() }}</h3>
                <p class="text-gray-600">Agents en congé maladie</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border">
        <form method="GET" class="flex items-center space-x-4">
            ...
            @if(request('search'))
                <a href="{{ route('agents.malades') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Grade/Funcition</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date de Maladie</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            @if($agent->date_maladie)
                                {{ $agent->date_maladie->diffInDays(now()) }} jours
                            @else
                                -
                            @endif
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                            Aucun agent en congé maladie trouvé.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
