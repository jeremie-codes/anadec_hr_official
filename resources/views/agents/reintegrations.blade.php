...
@section('title', 'Réintégrations - ANADEC RH')
@section('page-title', 'Réintégrations')
@section('page-description', 'Liste des agents réintégrés')
...
<div class="space-y-6">
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border">
        <div class="flex items-center">
            <div class="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center mr-4">
                <i class="bx bx-undo text-green-600 text-2xl"></i>
            </div>
            <div>
                <h3 class="text-2xl font-bold text-gray-900">{{ $agents->total() }}</h3>
                <p class="text-gray-600">Réintégrations enregistrées</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow-sm border">
        <form method="GET" class="flex items-center space-x-4">
            ...
            @if(request('search'))
                <a href="{{ route('agents.reintegrations') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date de Réintégration</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $agent->date_reintegration ? $agent->date_reintegration->format('d/m/Y') : '-' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-900">
                            {{ $agent->motif_changement_statut ?: '-' }}
                        </td>
