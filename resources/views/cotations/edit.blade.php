...
@section('title', 'Modification Cotation - ANADEC RH')
@section('page-title', 'Modification Cotation')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select name="agent_id" id="agent_id" required disabled onchange="calculerCotation()"
                            ...
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}" {{ old('agent_id', $agent->id) == $agent->id ? 'selected' : '' }}>
                                    {{ $agent->full_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="periode_debut" id="periode_debut" required
                                   value="{{ old('periode_debut', $cotation->periode_debut->format('Y-m-d\TH:i')) }}"
                        ...
                        <div>
                            ...
                            <input type="date" name="periode_fin" id="periode_fin" required
                                   value="{{ old('periode_fin', $cotation->periode_fin->format('Y-m-d\TH:i')) }}"
                                   onchange="calculerCotation()"
                                   class="mt-1 py-2 px-4 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-anadec-blue focus:border-anadec-blue">
                            @error('periode_fin')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="observations" id="observations" rows="4"
                                  class="mt-1 py-2 px-4 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-anadec-blue focus:border-anadec-blue"
                                  placeholder="Commentaires sur la performance de l'agent...">{{ old('observations', $cotation->observation) }}</textarea>
                        @error('observations')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
